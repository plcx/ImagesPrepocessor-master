...
def check_new_training_evaluating_data():
    ...
    # ==============================================
    root_path = r'E:\NucleuLabeling\NucSegTraining'

    rawnuc_niigz_paths = glob.glob(os.path.join(root_path, 'RawNuc', '*rawNuc.nii.gz'))

    segcell_niigz_paths = glob.glob(os.path.join(root_path, 'SegFullSphereNuc', '*segFullNuc.nii.gz'))


    for niigz in rawnuc_niigz_paths:
        embryo_name, tp = os.path.basename(niigz).split('.')[0].split('_')[:2]
        print(embryo_name, tp)
        ...
        seg_nuc_path = os.path.join(root_path, 'SegFullSphereNuc', '{}_{}_segFullNuc.nii.gz'.format(embryo_name, tp))
        seg_nuc_shape = nib_load(seg_nuc_path).shape


        assert raw_nuc_shape == seg_nuc_shape
        print(raw_nuc_shape, seg_nuc_shape)
